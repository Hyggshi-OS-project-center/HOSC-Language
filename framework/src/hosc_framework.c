...
static int execute_program(const Program* program, int* touched_gui) {
    ...
    while (pc < program->count && !should_quit) {
        ...
        if (*touched_gui && hosc_gui_backend() == HOSC_GUI_BACKEND_WIN32 && !hosc_gui_is_running()) {
            should_quit = 1;
            break;
        }
        ...
        switch (inst->opcode) {
            case OP_WINDOW:
                hosc_gui_create_window(inst->text, inst->a, inst->b);
                *touched_gui = 1;
...
#ifdef _WIN32
                MessageBoxA(NULL, inst->text, "HOSC", MB_OK | MB_ICONINFORMATION);
#else
            ...
            case OP_LOOP_SIMPLE: {
                ...
                while (!should_quit) {
                    ...
                    if (hosc_gui_backend() == HOSC_GUI_BACKEND_WIN32 && !hosc_gui_is_running()) {
                        should_quit = 1;
                        break;
                    }
