...
- `H000`: source file could not be read (`cannot open source file`) or import resolution failed (`failed to resolve import`)
- `H001`: missing `func main()` entry point
- `H002`: general syntax parse error (fixed 2026-07-31: previously could surface uninitialized heap garbage as the message and `1:1` as a bogus location when a parse function returned `NULL` without calling `parser_set_error`; `parser_create` now zero-initializes the `Parser` struct)
- `H003`: framework script detected, should be run through `framework/bin/hosc_framework.exe`
- `H104`: unknown keyword or keyword typo with recommendation (`did you mean 'var'?`) and token underline (`^~~~~`)
- `H201`: undeclared identifier with closest match recommendation
- `H202`: duplicate symbol declaration in same scope
- `H203`: assignment to `const` symbol
- `H204`: type mismatch in binary operator (e.g. `string - int`)
- `H205`: calling a non-function identifier or unknown function
- `H206`: function argument count mismatch
- `H900`: bytecode emission or allocation failure

`compiler/src/sema/type_checker.c` implements a two-pass semantic analyzer with a nested scope symbol table.
...
- the LSP diagnostics parser expects `file:line:col:` style output, which is now produced by the CLI diagnostic renderer (`file:line:col: error CODE:`)
