...
static char* join_path(const char* base, const char* child) {
    ...
    if (base_len > 0 && !is_path_separator(base[base_len - 1])) {
        /* A forward slash is accepted by both Windows and POSIX file APIs. */
...
static char g_last_resolution_error[512];
static char g_last_resolution_file[512];
static int g_last_resolution_line = 1;
...
static char* resolve_imports_recursive(const char* path, PathList* active, PathList* loaded, int is_entry) {
    ...
    if (!source) {
        if (g_last_resolution_error[0] == '\0') {
            snprintf(g_last_resolution_file, sizeof(g_last_resolution_file), "%s", path);
            g_last_resolution_line = 1;
            if (is_entry) {
                snprintf(g_last_resolution_error, sizeof(g_last_resolution_error),
    ...
    while (*cursor) {
        ...
        if (module) {
            ...
            if (!imported) {
                snprintf(g_last_resolution_file, sizeof(g_last_resolution_file), "%s", path);
                g_last_resolution_line = line_no;
                snprintf(g_last_resolution_error, sizeof(g_last_resolution_error),
...
static char* resolve_imports_for_file(const char* path) {
    ...
    PathList loaded = {0};
    char* out;
    g_last_resolution_error[0] = '\0';
    g_last_resolution_file[0] = '\0';
    g_last_resolution_line = 1;

    out = resolve_imports_recursive(path, &active, &loaded, 1);
...
static int emit_jump(HbcEmitter* e, HOpcode op, size_t* operand_offset) {
    if (!emit_op(e, op)) return 0;
    *operand_offset = e->code_len;
    return emit_u16(e, 0);
}

static int patch_jump(HbcEmitter* e, size_t operand_offset, size_t target) {
    size_t relative_target;
    if (target < e->function_start || operand_offset + 1 >= e->code_len) return 0;
    relative_target = target - e->function_start;
    if (relative_target > 0xffff) return 0;
    e->code[operand_offset] = (uint8_t)(relative_target & 0xffu);
    e->code[operand_offset + 1] = (uint8_t)((relative_target >> 8) & 0xffu);
    return 1;
}
...
static int add_constant_float(HBytecode* bc, double value) {
    HBCConstant* next;
    size_t index = bc->constant_count;
    next = (HBCConstant*)realloc(bc->constants, (index + 1) * sizeof(HBCConstant));
    if (!next) return -1;
    bc->constants = next;
    bc->constants[index].tag = HBC_CONST_FLOAT;
    bc->constants[index].as.float_value = value;
    bc->constant_count++;
    return (int)index;
}
...
static int find_local(FunctionContext* ctx, const char* name) {
    size_t i;
    if (!ctx || !name) return -1;
    for (i = 0; i < ctx->count; ++i) {
        if (ctx->names[i] && strcmp(ctx->names[i], name) == 0) return (int)i;
    }
    return -1;
}

static int compile_expression(HbcEmitter* e, FunctionContext* ctx, ASTNode* expr);

static int compile_call(HbcEmitter* e, FunctionContext* ctx, ASTNode* expr) {
    ASTNodeList* arg;
    int global_index;
    int argc = 0;
    ...
    for (arg = expr->data.call_expr.arguments; arg; arg = arg->next) {
        if (!compile_expression(e, ctx, arg->node)) return 0;
        argc++;
    }
...
static int compile_expression(HbcEmitter* e, FunctionContext* ctx, ASTNode* expr) {
    ...
    switch (expr->type) {
        ...
        case AST_NUMBER:
            ...
            return emit_indexed(e, OP_CONSTANT, index);
        case AST_FLOAT:
            index = add_constant_float(e->bc, expr->data.fnumber.value);
            return emit_indexed(e, OP_CONSTANT, index);
        case AST_BOOL:
            return emit_op(e, expr->data.boolean.value ? OP_TRUE : OP_FALSE);
        case AST_IDENTIFIER:
            index = find_local(ctx, expr->data.identifier.name);
            if (index >= 0) return emit_indexed(e, OP_GET_LOCAL, index);
            return emit_op(e, OP_NIL);
        case AST_CALL_EXPR:
            return compile_call(e, ctx, expr);
        case AST_UNARY_OP:
            if (!compile_expression(e, ctx, expr->data.unary_op.operand)) return 0;
            ...
            return emit_op(e, OP_NIL);
        case AST_BINARY_OP:
            if (!compile_expression(e, ctx, expr->data.binary_op.left)) return 0;
            if (!compile_expression(e, ctx, expr->data.binary_op.right)) return 0;
            switch (expr->data.binary_op.op) {
                ...
                case TOKEN_LESS: return emit_op(e, OP_LT);
                case TOKEN_LESS_EQUAL: return emit_op(e, OP_LE);
                case TOKEN_GREATER: return emit_op(e, OP_GT);
                case TOKEN_GREATER_EQUAL: return emit_op(e, OP_GE);
                default: return 0;
            }
        default:
            return emit_op(e, OP_NIL);
...
static int compile_statement(HbcEmitter* e, FunctionContext* ctx, ASTNode* stmt);

static int compile_block(HbcEmitter* e, FunctionContext* ctx, ASTNode* block) {
    ASTNodeList* cur;
    if (!block || block->type != AST_BLOCK) return 0;
    for (cur = block->data.block.statements; cur; cur = cur->next) {
        if (!compile_statement(e, ctx, cur->node)) return 0;
    }
    return 1;
}
...
static int compile_statement(HbcEmitter* e, FunctionContext* ctx, ASTNode* stmt) {
    ...
    switch (stmt->type) {
        case AST_BLOCK:
            return compile_block(e, ctx, stmt);
        case AST_VARIABLE_DECLARATION: {
            int slot;
            if (ctx->count >= 256) return 0;
            slot = find_local(ctx, stmt->data.variable_declaration.identifier);
            if (slot < 0) {
                slot = ctx->count;
                ctx->names[ctx->count++] = stmt->data.variable_declaration.identifier;
            }
            if (!compile_expression(e, ctx, stmt->data.variable_declaration.value)) return 0;
            if (!emit_indexed(e, OP_SET_LOCAL, slot)) return 0;
            return emit_op(e, OP_POP);
        }
        case AST_ASSIGNMENT: {
            int slot = find_local(ctx, stmt->data.assignment.identifier);
            if (slot < 0 || !compile_expression(e, ctx, stmt->data.assignment.value)) return 0;
            if (!emit_indexed(e, OP_SET_LOCAL, slot)) return 0;
            return emit_op(e, OP_POP);
        }
        case AST_PRINT_STATEMENT:
            ...
            if (!compile_expression(e, ctx, stmt->data.print_statement.expression)) return 0;
            if (!emit_indexed(e, OP_CALL, 1)) return 0;
            return emit_op(e, OP_POP);
        case AST_EXPR_STATEMENT:
            if (!compile_expression(e, ctx, stmt->data.expr_stmt.expression)) return 0;
            return emit_op(e, OP_POP);
        case AST_IF: {
            size_t else_jump;
            size_t end_jump;
            if (!compile_expression(e, ctx, stmt->data.if_stmt.condition) ||
                !emit_jump(e, OP_JUMP_IF_FALSE, &else_jump) ||
                !compile_block(e, ctx, stmt->data.if_stmt.then_branch)) return 0;
            if (stmt->data.if_stmt.else_branch) {
                if (!emit_jump(e, OP_JUMP, &end_jump) ||
                    !patch_jump(e, else_jump, e->code_len) ||
                    !compile_statement(e, ctx, stmt->data.if_stmt.else_branch) ||
                    !patch_jump(e, end_jump, e->code_len)) return 0;
            } else if (!patch_jump(e, else_jump, e->code_len)) {
                return 0;
            }
            return 1;
        }
        case AST_FOR: {
            ...
            if (stmt->data.for_stmt.init && !compile_statement(e, ctx, stmt->data.for_stmt.init)) return 0;
            loop_start = e->code_len;
            if (stmt->data.for_stmt.condition) {
                if (!compile_expression(e, ctx, stmt->data.for_stmt.condition) ||
                    !emit_jump(e, OP_JUMP_IF_FALSE, &exit_jump)) return 0;
            }
            if (!compile_block(e, ctx, stmt->data.for_stmt.body)) return 0;
            if (stmt->data.for_stmt.update && !compile_statement(e, ctx, stmt->data.for_stmt.update)) return 0;
            if (!emit_jump(e, OP_JUMP, &loop_jump) || !patch_jump(e, loop_jump, loop_start)) return 0;
            if (stmt->data.for_stmt.condition && !patch_jump(e, exit_jump, e->code_len)) return 0;
            return 1;
        }
        case AST_RETURN:
            if (stmt->data.return_stmt.value) {
                if (!compile_expression(e, ctx, stmt->data.return_stmt.value)) return 0;
        ...
        case AST_SWITCH: {
            ...
            int switch_slot;
            if (ctx->count >= 256) return 0;
            switch_slot = ctx->count++;
            ctx->names[switch_slot] = "<switch>";
            if (!compile_expression(e, ctx, stmt->data.switch_stmt.expression) ||
                !emit_indexed(e, OP_SET_LOCAL, switch_slot) || !emit_op(e, OP_POP)) return 0;
            for (item = stmt->data.switch_stmt.cases; item; item = item->next) {
                ASTNode* case_node = item->node;
                size_t next_case;
                if (!case_node->data.case_stmt.value) { default_body = case_node->data.case_stmt.body; continue; }
                if (!emit_indexed(e, OP_GET_LOCAL, switch_slot) ||
                    !compile_expression(e, ctx, case_node->data.case_stmt.value) ||
                    !emit_op(e, OP_EQ) || !emit_jump(e, OP_JUMP_IF_FALSE, &next_case) ||
                    !compile_block(e, ctx, case_node->data.case_stmt.body)) { free(end_jumps); return 0; }
                if (end_count == end_cap) {
                    size_t next_cap = end_cap ? end_cap * 2 : 4;
                    size_t* next = (size_t*)realloc(end_jumps, next_cap * sizeof(size_t));
                    if (!next) { free(end_jumps); return 0; }
                    end_jumps = next; end_cap = next_cap;
                }
                if (!emit_jump(e, OP_JUMP, &end_jumps[end_count++]) || !patch_jump(e, next_case, e->code_len)) { free(end_jumps); return 0; }
            }
            if (default_body && !compile_block(e, ctx, default_body)) { free(end_jumps); return 0; }
            for (size_t i = 0; i < end_count; ++i) if (!patch_jump(e, end_jumps[i], e->code_len)) { free(end_jumps); return 0; }
            free(end_jumps);
            return 1;
        }
        case AST_PACKAGE:
...
static HBytecode* hosc_build_ast_bytecode(ASTNode* ast) {
    ...
    for (i = 0; i < function_count; ++i) {
        ...
        ctx.function = fn;
        ctx.count = 1; /* Slot zero holds the callee. */
        for (param = 0; param < fn->data.function.param_count; ++param) {
            ctx.names[ctx.count++] = fn->data.function.params[param];
        }
        bc->functions[i].code_offset = (uint32_t)start;
        e.function_start = start;
        if (!compile_block(&e, &ctx, fn->data.function.body)) goto fail;
        if (e.code_len == start || e.code[e.code_len - 1] != (uint8_t)OP_RETURN) {
            if (!emit_op(&e, OP_NIL) || !emit_op(&e, OP_RETURN)) goto fail;
        }
        bc->functions[i].local_count = ctx.count;
...
HoscCompileResult hosc_compile_memory(
    ...
    if (hosc_looks_like_framework_script(source)) {
        hosc_diag_bag_add(
            ...
            display_path,
            (HoscSourceSpan){1, 1, 1, 1},
            "detected framework GUI script. Run it with the platform-native framework/bin/hosc_framework executable");
        return result;
    }

    ast = parser_parse(source);
    if (!ast) {
        const char* parse_error = parser_get_last_error();
        int err_line = 1, err_col = 1, err_end_line = 1, err_end_col = 1;
        const char* code = "H002";
        parser_get_last_error_span(&err_line, &err_col, &err_end_line, &err_end_col);
        if (parse_error) {
            if (strstr(parse_error, "unknown function")) {
                code = "H205";
            } else if (strstr(parse_error, "unknown keyword") || strstr(parse_error, "unexpected")) {
                code = "H104";
            }
        }
        ...
        hosc_diag_bag_add(
            ...
            code,
            display_path,
            (HoscSourceSpan){err_line, err_col, err_end_line, err_end_col},
            parse_error ? parse_error : "parse failed");
        return result;
    }

    if (!hosc_type_check(ast, diagnostics, display_path)) {
        ast_release_arena();
        return result;
    }
...
HoscCompileResult hosc_compile_file(const char* path, const HoscCompileOptions* options) {
    ...
    if (!source) {
        HDiagnosticBag* diagnostics;
        const char* err = g_last_resolution_error[0] ? g_last_resolution_error : "failed to read source file or resolve imports";
        const char* err_file = g_last_resolution_file[0] ? g_last_resolution_file : path;
        int err_line = g_last_resolution_line > 0 ? g_last_resolution_line : 1;
        ...
        if (diagnostics) {
            ...
            hosc_diag_bag_add(
                ...
                "H000",
                err_file,
                (HoscSourceSpan){err_line, 1, err_line, 1},
                err);
        }
        return result;
    }
