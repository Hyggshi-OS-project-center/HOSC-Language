...
$linkFlags = if ($isWindowsHost) { @('-lwinmm') } else { @() }
$archiveFlags = @('rcs')
...
function Resolve-RepoPath([string]$RelativePath) {
    ...
    return Join-Path $root $portablePath
}
...
function Build-TargetSet {
    ...
    $cliExe = New-Executable "hosc$executableExtension" $cliObjects @($compilerLib, $runtimeLib, $vmLib)
    $hostExe = New-Executable "hvm_host$executableExtension" $runtimeHostObjects @($runtimeLib, $vmLib)
...
function Sync-LegacyToolsBin([string]$CliExe, [string]$HostExe) {
    ...
    $legacyCli = Join-Path $legacyBinDir "hosc$executableExtension"
    $legacyHvm = Join-Path $legacyBinDir "hvm$executableExtension"
    $legacyHost = Join-Path $legacyBinDir "hvm_host$executableExtension"
...
function Invoke-SmokeTests([string]$CliExe) {
    $helloPath = Resolve-RepoPath 'framework/examples/Hello.hosc'
    $helloBytecode = Resolve-RepoPath 'framework/examples/Hello.hbc'
