...
$buildScript = Join-Path (Join-Path $root 'tools') 'build.ps1'
$toolsBin = Join-Path (Join-Path $root 'tools') 'bin'
$hosc = Join-Path $toolsBin "hosc$executableExtension"
$hvm = Join-Path $toolsBin "hvm$executableExtension"
$hvmHost = Join-Path $toolsBin "hvm_host$executableExtension"
$gateDir = Join-Path (Join-Path $root 'build') 'quality_gate'
