...
class HOSCIDEMain(QMainWindow):
    def __init__(self):
        ...
        self.runtime_host = self.project_root / "tools" / "bin" / "hvm.exe"
        self.framework_runner = self.project_root / "framework" / "bin" / "hosc_framework.exe"
        self.build_dir = self.project_root / "tools" / "bin"
    ...
    def ensure_cli(self):
        if not self.cli.exists():
            QMessageBox.critical(
                self,
                "CLI missing",
                "Missing tools/bin CLI. Run Build -> Build Toolchain or tools/build.ps1 first:\n\n" + str(self.cli),
            )
            return False
        return True
    ...
    def ensure_runtime_host(self):
        if not self.runtime_host.exists():
            QMessageBox.critical(
                ...
                "Missing hvm.exe. Run Build -> Build Toolchain or tools/build.ps1 first:\n\n" + str(self.runtime_host),
            )
            return False
        return True
