...
def make_mono_font(size):
    font = QFont()
    try:
        font.setFamilies(["JetBrains Mono", "Cascadia Code", "Consolas"])
...
FONT_MONO = make_mono_font(12)
FONT_MONO_SM = make_mono_font(10)
...
class CodeEditor(QTextEdit):
    def __init__(self, completion_words):
        ...
        self.setFont(QFont(FONT_MONO))
        self.document().setDefaultStyleSheet("p { line-height: 140%; }")
...
class HOSCIDEMain(QMainWindow):
    def __init__(self):
        ...
        self.executable_suffix = ".exe" if sys.platform == "win32" else ""
        self.shell_name = "PowerShell" if sys.platform == "win32" else "Shell"
        self.examples_dirs = [
            self.project_root / "framework" / "examples",
            self.project_root / "examples" / "level_a",
            self.project_root / "examples",
        ]
        self.build_dir = self.project_root / "build" / "cmake"
        self.framework_runner = self.project_root / "framework" / "bin" / f"hosc_framework{self.executable_suffix}"
    ...
    def init_ui(self):
        ...
        self.explorer_header = QLabel("EXPLORER")
        self.explorer_header.setObjectName("explorerHeader")

        self.file_list = QListWidget()
        self.file_list.setObjectName("explorerList")
        ...
        sidebar_layout.addWidget(self.explorer_header)
        ...
        self.output.setFont(QFont(FONT_MONO_SM))

        self.terminal_output = QTextEdit()
        self.terminal_output.setObjectName("terminalView")
        self.terminal_output.setReadOnly(True)
        self.terminal_output.setFont(QFont(FONT_MONO_SM))

        self.terminal_input = QLineEdit()
        self.terminal_input.setObjectName("terminalInput")
        self.terminal_input.setFixedHeight(30)
        self.terminal_input.setPlaceholderText(
            f"{self.shell_name} command. Press Enter to run. Use 'cd <path>' to change terminal folder."
        )
        self.terminal_input.returnPressed.connect(self.run_terminal_command)

        self.terminal_run_btn = QPushButton("Run")
        self.terminal_run_btn.setObjectName("terminalRunButton")
        self.terminal_run_btn.setFixedWidth(80)
        self.terminal_run_btn.clicked.connect(self.run_terminal_command)

        self.terminal_clear_btn = QPushButton("Clear")
        self.terminal_clear_btn.setObjectName("terminalClearButton")
        self.terminal_clear_btn.setFixedWidth(80)
        self.terminal_clear_btn.clicked.connect(self.clear_terminal_output)

        self.terminal_cwd_label = QLabel()
        self.terminal_cwd_label.setObjectName("terminalCwdLabel")
        self.update_terminal_cwd_label()

        terminal_input_row = QHBoxLayout()
        terminal_input_row.setContentsMargins(8, 8, 8, 8)
        terminal_input_row.setSpacing(8)
        terminal_input_row.addWidget(self.terminal_cwd_label, 0)
        terminal_input_row.addWidget(self.terminal_input, 1)
        terminal_input_row.addWidget(self.terminal_run_btn)
        terminal_input_row.addWidget(self.terminal_clear_btn)
        ...
        self.status_file_label.setObjectName("statusFileLabel")
        self.status_file_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.status_file_label.setFont(QFont(FONT_MONO_SM))

        self.status_cursor_label = QLabel("Ln -, Col -")
        self.status_cursor_label.setObjectName("statusCursorLabel")
        self.status_cursor_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        self.status_cursor_label.setFont(QFont(FONT_MONO_SM))

        status.addWidget(self.status_file_label, 1)
        status.addPermanentWidget(self.status_cursor_label, 0)
    ...
    def apply_style(self):
        self.setStyleSheet(
            ...
            QTextEdit#outputView, QTextEdit#terminalView {{
                background: {BG_PANEL};
                color: {TEXT_MED};
                border: none;
            }}
    ...
    def update_terminal_cwd_label(self):
        parts = Path(self.terminal_cwd).parts
        if len(parts) > 2:
            display = ".../" + "/".join(parts[-2:])
        else:
            display = str(self.terminal_cwd)
        self.terminal_cwd_label.setText(display)
    ...
    def run_terminal_command(self):
        ...
        prompt = "PS" if sys.platform == "win32" else "$"
        self.append_terminal_log(f"{prompt} {self.terminal_cwd}> {cmd_text}")
    ...
    def update_status(self, editor=None):
        ...
        if editor is None:
            ...
            self.status_cursor_label.setText("Ln -, Col -")
            return
        ...
        self.status_file_label.setText(f"{name} | {state}")
        self.status_cursor_label.setText(f"Ln {line}, Col {col}")
    ...
    def ensure_cli(self):
        ...
        if not self.cli.exists():
            QMessageBox.critical(
                self,
                "CLI missing",
                "Missing CLI. Run Build -> Build Toolchain first:\n\n" + str(self.cli),
            )
            return False
        return True
    ...
    def ensure_runtime_host(self):
        ...
        if not self.runtime_host.exists():
            QMessageBox.critical(
                ...
                "Missing runtime host. Run Build -> Build Toolchain first:\n\n" + str(self.runtime_host),
            )
            return False
        return True
    ...
    def has_framework_markers(self, text):
        patterns = [
            r"\bwindow\s*\(",
            r"\btext\s*\(",
            r"\bpump_events\s*\(",
    ...
    def apply_error_underlines(self, editor, lines):
        ...
        for line_no in editor.error_lines:
            ...
            fmt.setUnderlineColor(QColor(ERROR_RED))
    ...
    def build_toolchain(self):
        configure = self.run_subprocess_logged([
            "cmake", "-S", str(self.project_root), "-B", str(self.build_dir), "-DHOSC_BUILD_TESTS=ON"
        ])
        if configure.returncode != 0:
            QMessageBox.warning(self, "Build Toolchain", f"CMake configuration failed (exit {configure.returncode}).")
            return False

        result = self.run_subprocess_logged(["cmake", "--build", str(self.build_dir), "--parallel"])
    ...
    def refresh_tool_paths(self):
        ...
        def find_binary(name):
            filename = f"{name}{self.executable_suffix}"
            candidates = [bin_dir / filename]
            candidates.extend(bin_dir / config / filename for config in ("Debug", "Release", "RelWithDebInfo", "MinSizeRel"))
            return next((path for path in candidates if path.exists()), candidates[0])

        self.cli = find_binary("hosc")
        self.runtime_host = find_binary("hvm_host")
    ...
    def run_file(self):
        ...
        use_framework = self.is_framework_source(src) or self.has_framework_markers(editor.toPlainText())
