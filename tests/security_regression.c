...
static int test_parser_depth_limit(void) {
    char source[2048];
    int i;
    int depth = 300;
    ASTNode *ast;
    ...
    if (ast) {
        free_ast(ast);
        return expect_true(0, "deeply nested syntax should be rejected by parser depth limit");
    }
    return expect_true(parser_get_last_error() != NULL, "parser should set error diagnostic on max depth limit");
}
...
int main(void) {
    ...
    failures += test_parser_depth_limit();
