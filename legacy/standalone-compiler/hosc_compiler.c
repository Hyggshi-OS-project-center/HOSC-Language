...
static int run_c_compiler(const char *c_path, const char *exe_path) {
    if (!is_safe_path(c_path) || !is_safe_path(exe_path)) return 0;

    const char *argv[] = { "gcc", "-O2", "-std=c99", c_path, "-o", exe_path, NULL };
#if defined(_WIN32)
    intptr_t status = _spawnvp(_P_WAIT, "gcc", argv);
    return status == 0;
#else
    pid_t pid = fork();
    if (pid < 0) return 0;
...
#define MAX_BYTECODE_INSTRUCTIONS (10u * 1024u * 1024u)
#define MAX_BYTECODE_STRING_LEN (16u * 1024u * 1024u)

static void free_partially_read_instructions(HVM_Instruction *code, uint32_t loaded_count) {
    uint32_t i;
    if (!code) return;
...
static HVM_Instruction *read_bytecode(const char *path, size_t *out_count) {
    ...
    if (count > MAX_BYTECODE_INSTRUCTIONS || count > ((size_t)-1) / sizeof(HVM_Instruction)) {
        fclose(fp);
        return NULL;
    }
    ...
    if (fseek(fp, 8, SEEK_SET) != 0) { fclose(fp); return NULL; }
    ...
    for (i = 0; i < count; i++) {
        uint8_t op, kind;
        long current_pos;
        if (fread(&op, 1, 1, fp) != 1) { free_partially_read_instructions(code, i); fclose(fp); return NULL; }
        if (fread(&kind, 1, 1, fp) != 1) { free_partially_read_instructions(code, i); fclose(fp); return NULL; }
        code[i].opcode = (HVM_Opcode)op;
        switch (kind) {
            case OP_INT:
                if (fread(&code[i].operand.int_operand, sizeof(int64_t), 1, fp) != 1) {
                    free_partially_read_instructions(code, i); fclose(fp); return NULL;
                }
                break;
            case OP_FLOAT:
                if (fread(&code[i].operand.float_operand, sizeof(double), 1, fp) != 1) {
                    free_partially_read_instructions(code, i); fclose(fp); return NULL;
                }
                break;
            case OP_STRING: {
                ...
                if (len > MAX_BYTECODE_STRING_LEN || len >= ((size_t)-1) - 1) {
                    free_partially_read_instructions(code, i); fclose(fp); return NULL;
                }
                current_pos = ftell(fp);
                if (current_pos < 0 || (size_t)(file_size - current_pos) < len) {
                    free_partially_read_instructions(code, i); fclose(fp); return NULL;
                }
                ...
                if (len > 0) {
                    if (fread(code[i].operand.string_operand, 1, len, fp) != len) {
                        free_partially_read_instructions(code, i + 1); fclose(fp); return NULL;
                    }
